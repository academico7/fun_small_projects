...
int partition (char **arr, wordcounter &wc, int lowLimit, int high)
{
...
void quickSort(char **arr, wordcounter &wc, int lowLimit, int highLimit)
{
...
int main() {
  ...
  char wrd[30];
  char ** mostCommon = new char *[NRCOMMONWORDS]; // 20 most common words
  short int m_size = 0;
  ...
  if ( !f_in )  {
    ...
    if ( proc_w != "_invalid_" )  {
      ...
      else if (wordmap.get(proc_w) > wordmap.get( mostCommon[ 0 ] ))  {
  ...
  for( int i = m_size-1; i >= 0 ; i--)  {
    std::cout << wordmap.get(mostCommon[i]) << " - " << mostCommon[i]  << std::endl;
  }

  // Memory clean-up
  delete [] mostCommon; 

  // Performance of 0.59 seconds
